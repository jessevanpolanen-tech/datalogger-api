...
// Simulated dataloggers with their current states
const dataloggers = {
  'DL-001': { temp: 22.5, humidity: 55, lastUpdate: Date.now() },
  'DL-002': { temp: 21.8, humidity: 58, lastUpdate: Date.now() },
  'DL-003': { temp: 23.2, humidity: 52, lastUpdate: Date.now() },
  'DL-004': { temp: 20.5, humidity: 60, lastUpdate: Date.now() },
  'DL-005': { temp: 24.1, humidity: 48, lastUpdate: Date.now() }
};

// Temperature and humidity thresholds for deviation detection
const TEMP_MIN = 18;
const TEMP_MAX = 25;
...
function updateSensorReadings() {
  Object.keys(dataloggers).forEach(id => {
    ...
    // Add small random fluctuations (-0.5 to +0.5 for temp, -2 to +2 for humidity)
...
app.get('/api/datalogger', (req, res) => {
  ...
  const deviation = checkDeviation(logger.temp, logger.humidity);
